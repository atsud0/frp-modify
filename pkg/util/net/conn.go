...
//添加host选项，配合使用
func ConnectServer(protocol string, addr string, host string) (c net.Conn, err error) {
	switch protocol {
	case "tcp":
	...
	case "websocket":
		return DialWebsocketServer(addr, host)
	default:
...
// addr: domain:port
func DialWebsocketServer(addr string, host string) (net.Conn, error) {
